...
#include "psiswarm_proximity_default_sensor.h"

namespace argos
{
    static CRange<Real> UNIT(0.0f, 1.0f);

    CPsiSwarmProximityDefaultSensor::CPsiSwarmProximityDefaultSensor():
    ...
    void CPsiSwarmProximityDefaultSensor::Init(TConfigurationNode& t_tree)
    {
        try
        {
            CCI_PsiSwarmProximitySensor::Init(t_tree);
            ...
            if(fNoiseLevel < 0.0f)
            {
                THROW_ARGOSEXCEPTION("Can't specify a negative value for the noise level of the psi-swarm proximity sensor");
            }
        ...
        {
            THROW_ARGOSEXCEPTION_NESTED("Initialization error in psi-swarm default proximity sensor", ex);
        }
    }

    void CPsiSwarmProximityDefaultSensor::SetRobot(CComposableEntity& c_entity)
    {
        try
        {
        ...
        {
            THROW_ARGOSEXCEPTION_NESTED("Can't set robot for the psi-swarm proximity default sensor", ex);
        }
    }

    void CPsiSwarmProximityDefaultSensor::Update()
    {
        ...
        for(UInt32 i = 0; i < m_tReadings.size(); ++i)
        {
            ...
            if(GetClosestEmbodiedEntityIntersectedByRay(sIntersection, cScanningRay, *m_pcEmbodiedEntity))
            {
    ...
    void CPsiSwarmProximityDefaultSensor::Reset()
    {
        for(UInt32 i = 0; i < GetReadings().size(); ++i)
            m_tReadings[i].Value = 0.0f;
    }

    // TODO: Update this function to accurately model the real Psi Swarm's proximity sensor readings
    Real CPsiSwarmProximityDefaultSensor::CalculateReading(Real f_distance)
    {
        Real value = 0.0f;
    ...
    // TODO: Update this description once CalculateReading() is changed
    REGISTER_SENSOR(CPsiSwarmProximityDefaultSensor,
                    "psiswarm_proximity", "default",
                    "York Robotics Laboratory",
                    "1.0",
                    "The Psi Swarm proximity sensor",
                    ...
                    "For usage, refer to [ci_psiswarm_proximity_sensor.h]\n\n"
                    ...
                    "         <sensors>\n"
                    "            ...\n"
                    "            <psiswarm_proximity implementation=\"default\"/>\n"
                    "            ...\n"
                    "         <sensors/>\n"
                    "         ...\n"
                    "      <my_controller/>\n"
                    ...
                    "         <sensors>\n"
                    "            ...\n"
                    "            <psiswarm_proximity implementation=\"default\" noise_level=\"0.1\"/>\n"
                    "            ...\n"
                    "         <sensors/>\n"
                    "         ...\n"
                    "      <my_controller/>\n"
